 ...
 *  - angles : orientation
 *  Outputs:
 *  - rot : body 2 ground rotation matrix
 */

void rotation_matrix( double *rot, double* angles){
    rot[0] = cos(angles[2]) * cos(angles[1]);
    rot[3] = -sin(angles[2]) * cos(angles[0]) + cos(angles[2]) * sin(angles[1]) * sin(angles[0]);
    rot[6] = sin(angles[2]) * sin(angles[0]) + cos(angles[2]) * sin(angles[1]) * cos(angles[0]);
    rot[1] = sin(angles[2]) * cos(angles[1]);
    rot[4] = cos(angles[2]) * cos(angles[0]) + sin(angles[2]) * sin(angles[1]) * sin(angles[0]);
    rot[7] = -cos(angles[2]) * sin(angles[0]) + sin(angles[2]) * sin(angles[1]) * cos(angles[0]);
    rot[2] = -sin(angles[1]);
    rot[5] = cos(angles[1]) * sin(angles[0]);
